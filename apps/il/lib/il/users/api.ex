defmodule Il.Users.API do
  ...
  defp get_expiration_date(request) do
    ttl = Confex.fetch_env!(:il, :credentials_recovery_request_ttl)
  ...
  defp send_email(email, %CredentialsRecoveryRequest{} = request) do
    case Il.Man.Templates.CredentialsRecoveryRequest.render(request) do
      {:ok, body} ->
        Il.Bamboo.Emails.CredentialsRecoveryRequest.send(email, body)
        :ok
