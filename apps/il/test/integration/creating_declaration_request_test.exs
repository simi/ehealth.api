defmodule Il.Integration.DeclarationRequestCreateTest do
  @moduledoc false

  use Il.Web.ConnCase, async: false
...
request. tax_id = #{conn.body_params["person"]["tax_id"]}</body></html>"
    ...
    test "declaration request is created with 'OTP' verification", %{conn: conn} do
      ...
      conn =
        ...
        |> Il.Web.ConnCase.put_client_id_header(legal_entity_id)
      ...
      declaration_request = Il.DeclarationRequest.API.get_declaration_request_by_id!(id)
      ...
      assert "CANCELLED" = Il.Repo.get(Il.DeclarationRequest, d1.id).status
      assert "CANCELLED" = Il.Repo.get(Il.DeclarationRequest, d2.id).status
    end
