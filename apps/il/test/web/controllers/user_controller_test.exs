defmodule Il.Web.UserControllerTest do
  @moduledoc false
  use Il.Web.ConnCase
  use Bamboo.Test
  ...
  describe "reset password" do
    ...
    test "returns validation error when request is expired", %{conn: conn} do
      ...
      old_ttl = Application.get_env(:il, :credentials_recovery_request_ttl)
      on_exit(fn ->
        Application.put_env(:il, :credentials_recovery_request_ttl, old_ttl)
      end)

      Application.put_env(:il, :credentials_recovery_request_ttl, 0)
      reset_attrs = %{"user" => %{"password" => "new_but_not_a_secret"}}
