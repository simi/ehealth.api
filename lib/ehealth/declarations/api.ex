defmodule EHealth.Declarations.API do
  ...
  def get_declarations(params, headers) do
    with {:ok, resp} <- OPS.get_declarations(params, headers),
         ...
         {divisions, _} <- Divisions.get_divisions(%{ids: division_ids}),
         {employees, _} <- Employees.get_employees(%{ids: employee_ids}),
         {legals, _} <- LegalEntities.get_legal_entities(%{ids: legal_entity_ids}),
